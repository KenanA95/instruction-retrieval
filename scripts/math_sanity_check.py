#!/usr/bin/env python
"""
Math task sanity check script.

This script runs a quick sanity check for the Math task to ensure that:
...
from src.tasks.math_mvp import run_math_experiment
...
def run_math_sanity_check(config_path=None, num_examples=2):
    """
    Run a sanity check for the Math task.
    ...
    # Load config
    if not config_path:
        config_path = os.path.join("configs", "math", "math_experiment_mvp.json")
    
    logger.info(f"Running Math sanity check with config from {config_path}")
    config = load_config(config_path)
    
    # Create a sanity check output directory
    original_output_dir = config.get("output_dir", "results/math")
    ...
    # Run the experiment
    try:
        logger.info(f"Running Math experiment with {num_examples} examples per variant")
        ...
        logger.info("Math sanity check completed successfully")
        return True
        
    except Exception as e:
        logger.error(f"Error during Math sanity check: {e}")
        return False

def main():
    parser = argparse.ArgumentParser(description="Run Math sanity check")
    parser.add_argument("--config", type=str, help="Path to config file")
    parser.add_argument("--num-examples", type=int, default=2, help="Number of examples to run per variant")
    
    args = parser.parse_args()
    
    success = run_math_sanity_check(args.config, args.num_examples)
    
    if success:
        logger.info("✅ Math sanity check PASSED")
        sys.exit(0)
    else:
        logger.error("❌ Math sanity check FAILED")
        sys.exit(1)
