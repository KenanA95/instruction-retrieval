...
parser.add_argument("--dataset", default="/shared/3/projects/instruction-retrieval/mathematics_dataset/processed/easy_100.tsv",
...
parser.add_argument("--dry-run", action="store_true",
                    help="Dry run - don't call OpenAI API, just print prompts")
...
# Set up OpenAI API key - only if not in dry run mode
if not args.dry_run:
...
def main():
    # Parse arguments again to get all arguments
    ...
    if not os.path.isabs(args.templates_dir):
        args.templates_dir = os.path.join(project_root, args.templates_dir)
    
    # Set up logging
    logger = setup_logger("generate_prompts")
    ...
    for topic in args.topics:
        ...
        for variant in args.variants:
            ...
            # Ensure variant directory exists in output dir
            ...
            # Run OpenAI to generate instructions or just print prompt in dry run mode
            if args.dry_run:
                logger.info(f"  Dry run - would generate instructions for {output_path}")
                logger.info(f"  Prompt: {prompt[:100]}...")
                
                # In dry run mode, save the prompt to a file for inspection
                with open(output_path + ".dry_run.txt", 'w') as f:
                    f.write(prompt)
                logger.info(f"  Saved prompt to {output_path}.dry_run.txt")
            else:
                try:
