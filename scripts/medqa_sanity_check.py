#!/usr/bin/env python
"""
MedQA task sanity check script.

This script runs a quick sanity check for the MedQA task to ensure that:
...
from src.tasks.medqa import run_medqa_experiment
...
def run_medqa_sanity_check(config_path=None, num_examples=2):
    """
    Run a sanity check for the MedQA task.
    ...
    # Load config
    if not config_path:
        config_path = os.path.join("configs", "medqa", "config.json")
    
    logger.info(f"Running MedQA sanity check with config from {config_path}")
    config = load_config(config_path)
    
    # Create a sanity check output directory
    original_output_dir = config.get("output_dir", "results/medqa")
    ...
    # Run the experiment
    try:
        logger.info(f"Running MedQA experiment with {num_examples} examples per variant")
        ...
        logger.info("MedQA sanity check completed successfully")
        return True
        
    except Exception as e:
        logger.error(f"Error during MedQA sanity check: {e}")
        return False

def main():
    parser = argparse.ArgumentParser(description="Run MedQA sanity check")
    parser.add_argument("--config", type=str, help="Path to config file")
    parser.add_argument("--num-examples", type=int, default=2, help="Number of examples to run per variant")
    
    args = parser.parse_args()
    
    success = run_medqa_sanity_check(args.config, args.num_examples)
    
    if success:
        logger.info("✅ MedQA sanity check PASSED")
        sys.exit(0)
    else:
        logger.error("❌ MedQA sanity check FAILED")
        sys.exit(1)
